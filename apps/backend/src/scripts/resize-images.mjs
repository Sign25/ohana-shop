...
import os from "os"
const SRC = "/srv/ohana/shared/images/detailed", DST = "/srv/ohana/shared/images/web"
...
async function one(src) {
  const rel = path.relative(SRC, src), out = path.join(DST, rel)
  try { const [a, b] = await Promise.all([fs.stat(src), fs.stat(out).catch(() => null)]); if (b && b.mtimeMs >= a.mtimeMs) { skipped++; return } } catch {}
  await fs.mkdir(path.dirname(out), { recursive: true })
  try {
    const ext = path.extname(src).toLowerCase()
    let img = sharp(src, { failOn: "none" }).rotate().resize({ width: MAX, height: MAX, fit: "inside", withoutEnlargement: true })
    if (ext === ".png") img = img.png({ compressionLevel: 9, palette: false }); else if (ext === ".webp") img = img.webp({ quality: Q }); else img = img.jpeg({ quality: Q, mozjpeg: true })
