...
export default async function FeaturedProducts({ countryCode }: { countryCode: string }) {
  ...
  const roots = categories.filter((c) => !c.parent_category_id && (c.metadata as any)?.kind !== "showcase" && (c.metadata as any)?.kind !== "manual-hits").slice(0, 5)
  ...
  const newest = dedup(fresh.ids.map((id) => freshById.get(id)).filter((p) => p && !(p.metadata as any)?.lineika) as typeof freshGot, 8)
