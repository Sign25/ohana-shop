...
## Порядок показа, скрытие и бейджи — правила старого сайта (перенесены 11.09.2026)

- **Сортировка по умолчанию — по названию** (как `default_products_sorting=product-asc` на старом сайте); подборка (`metadata.kind=showcase`) — «Рекомендуем» = ручной порядок `product.metadata.showcase_rank[handle]` (позиции витрины «Осень/зима 2027» перенесены `scripts/import-showcase-rank.ts` из cscart категории 560); тег «Новинки» — сначала новые; «Хиты» — по отзывам WB (ручные хиты без рейтинга — в середину, вес 3000). Дропдаун: По названию / Сначала новые / Хиты продаж / дешевле / дороже.
- **Главная:** секции по аудиториям — в наличии, по популярности (отзывы WB), одна модель — одна карточка (расцветки «Номенклатуры 2026» схлопываются по названию без кода); ряд «Новинки сезона» — поступления за 60 дней без комплектов, до 8 карточек, показывается при ≥4.
- **Скрытие:** без фото — нигде не показываем (списки + карточка); товар со статусом draft (архив 1С, блокировка 1С, скрыт вручную на старом сайте) или без фото → **302 в свой раздел** (`/store/ohana/gone`, главная категория каталога → живой родитель → /store), несуществующий адрес — 404. По умолчанию только в наличии, распроданные — в конец. **Галочка 1С «Блокировать выгрузку на сайт»** (регистр БУС_НоменклатураЗаблокированнаяКВыгрузкеНаСайт): `scripts/sync-1c-blocked.ts` раз в час (cron :27) → draft + `metadata.blocked_1c`; снятие галочки возвращает published только тем, кого отключил скрипт (32 товара при первом прогоне).
- **Даты создания со старого сайта** → `metadata.created_1c` (`scripts/set-old-timestamps.ts`, по cscart_key g<группа>/p<товар>); по ним считаются «Новинка» и тег «Новинки».
- **Бейджи** (`lib/util/ohana.ts: productBadges`, компонент `product-badges`): «Акция» отдельно слева вверху (#F4503A); справа внизу столбиком (bottom 12 %, капслок 11px, радиус 6px): **Хит** (#E69C4E: WB > 1000 отзывов и рейтинг > 4.7 ИЛИ категория «Хиты (ручной отбор)» → `metadata.hit_manual`, синк `scripts/sync-manual-hits.ts` в ночном конвейере; служебная категория создана, 18 артикулов со старого сайта), **Новинка** (#16a34a: создан после 28.06.2026 и не старше 21 дня, у комплектов не показываем), **Big size** (#246075, категория big-size), **Лето** (#0ea5e9, leto-2026), **Школа** (#4c6ef5, shkola), **Упаковка** (#5FA88C) | **Комплект** (#246075). Бейджа «Sale» (скидка CS-Cart от list price) на новом сайте нет — такого механизма нет. Рейтинг в плитке всегда занимает строку (пустые звёзды и 0), чтобы карточки не прыгали.
- **Главная категория товара** — профильная из каталога, не подборка (`mainCategory`): похожие товары и редирект скрытого товара считаются по ней.

## Меню, hero-карусель, хэштеги из 1С, блоки главной (11.09.2026, вечер)

- **Верхнее меню** — только разделы-аудитории (без подборок и «Розницы»). Подборки живут в хэштегах под баннером и в мобильном меню «Подборки»; ссылка на розничный магазин ohana.market — в блоке «Розница и маркетплейсы» на главной и в футере.
- **Hero-карусель** (`modules/home/components/hero-carousel`): один баннер в кадре, автопрокрутка 6 с, точки/стрелки/свайп; баннеры — из админки «Баннеры» либо два по умолчанию.
- **Хэштеги формируются автоматически**: динамические Новинки/Акции/Хиты (только если есть товары) → **теги 1С** (булевы свойства номенклатуры «Лето», «Школа», «Хит продаж», «Для дома», «Family Look», «8 марта»… — `product.metadata.tags`, порядок как в классификаторе 1С, служебные «Крупный опт/Всесезонный/Демисезонный» не показываем) → подборки-категории из 1С (Осень/зима 2027, Big size). Маршруты: `/store/ohana/tags` (теги с товарами в наличии), `/store/ohana/catalog?tag=…`, страница `/store?tag=Лето`. Теги пишет ночной `import-commerceml.ts` (свойство = «Да»); разовый перенос — `scripts/import-old-tags.ts` (старый сайт + текущая выгрузка). Теги также участвуют в бейджах (Хит продаж → Хит, Новинки/Поступления → Новинка, Лето, Школа).
- **Главная**: секции по всем разделам, включая «Продукты питания» (свой блок), «Новинки сезона» — последним блоком и без продуктов питания.

## Карточка товара в списках (редизайн 11.09.2026, `modules/products/components/product-preview`)

Порядок блоков: фото 3:4 (при наведении — второе фото товара и полоса «Выбрать размеры →» / «Заказать комплект →»; бейджи по правилам старого сайта; «Всё разобрали» слева вверху) → строка «Арт. … · цвет» → название (без дубля артикула/цвета и хвоста «(… в упаковке)», 2 строки на десктопе, 3 на телефоне) → рейтинг WB (строка всегда занята: «нет отзывов») → **полоска размеров** (в наличии — обычные чипы, закончились — перечёркнутые, максимум 8 + «+N»; у комплекта/упаковки — один чип «Комплект 46-56 · 6 шт» / «Упаковка 24 шт») → цена за штуку крупно (акция — терракотом, опт зачёркнут), стоимость комплекта/упаковки, «крупный опт N ₽» лазурью → подвал: точка статуса (зелёная / жёлтая «мало» = меньше 3 упаковок / серая) + «Все размеры» | «8 из 9 размеров» | «N комплектов» | «N упаковок», справа «упак. N шт». Списки берут `*images` (второе фото) и `*categories` (бейджи). Панель фильтров на телефоне свёрнута за кнопкой «Фильтры и размеры».

## Главная и подвал (11.09.2026, вечер)

Порядок блоков главной: hero-карусель (админка «Баннеры», место hero; перенесены баннеры старого сайта: Осень/зима, «И в пир», Новинки сезона, Лауреат премий — активны; Лето 2026, Весенняя распродажа, Распродажа полотенец — выключены, включаются в админке) → хэштеги → 4 преимущества с картинками (`home/usp`) → секции по разделам с двумя большими плитками подразделов (`home/section-tiles`, конфиг `lib/util/home-content.ts: SECTION_TILES`, подраздел ищется по названию) → рекламная полоса после «Мужчинам» (место strip: «Хиты продаж» активна, «Распродажа» выключена) → «Как начать работать с нами» после «Мальчикам» → Дом, Аксессуары, Продукты → Новинки сезона → Инструменты и «Розница и маркетплейсы» → «Нам доверяют» (бегущая строка логотипов партнёров) → SEO-блок с h1. Картинки старого сайта лежат в `/srv/ohana/shared/images/ohana/{usp,tiles,partners,hero/old}`.
Подвал (`layout/templates/footer`): бренд + награды + VK/OK, колонки Каталог / Покупателям / Инструменты + Компания, карточка контактов с кнопкой «Заказать звонок» и ссылкой на карту, строка «Розница и маркетплейсы», юридическая полоса (© ООО «Охана Маркет», Реквизиты, Оферта, Политика). Контакты и соцсети — `lib/util/home-content.ts: CONTACTS, SOCIALS`.
